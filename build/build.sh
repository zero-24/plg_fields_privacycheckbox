#!/bin/bash
EXTENSION_ZIP_FILENAME="build/plg_fields_privacycheckbox.zip"
EXTENSION_ELEMENT="privacycheckbox"
if [ ! -f "$EXTENSION_ELEMENT.xml" ]; then cd ..; fi
if [ -f "$EXTENSION_ZIP_FILENAME" ]; then rm $EXTENSION_ZIP_FILENAME; fi
zip -r $EXTENSION_ZIP_FILENAME fields/ language/ params/ tmpl/ "$EXTENSION_ELEMENT.php" "$EXTENSION_ELEMENT.xml" script.php --quiet
